...
async function doLogin(e) {
  ...
  try {
    ...
    if (!response.user) {
      err.style.display = 'block';
      err.textContent = 'Error: ' + (response.error || 'Credenciales inválidas');
      return;
    }
    ...
    updateUserUI(user);
    hideAuth();
    toast('✓ Sesión iniciada como ' + user.full_name, 's');
    try { await loadAll(); } catch(ex) { /* continúa aunque BD falle */ }
...
>>>>>>> 9f3a75ea28df563910c6c334a1612253d4f4a19c
